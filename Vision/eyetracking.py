...
def eyetracking():
    ...
    while True:
        ...
        for face in faces:
            ...
            # cv2.circle(img=frame, center=right_center, radius=3, color=(0, 255, 255), thickness=-1)
            ...
            # print("test x :",testxx.mean())
            # print("test y :",testyy.mean())
            # print(len(testxx))
            if (len(testxx) >= 4):
            ...
            ''' 임의로 정한 부정행위 값'''
            # print(testxx-1.5,",",testyy-1)
            if abs(testxx.mean() - 1) > 5 or abs(testyy.mean() - 1) > 1.5:
